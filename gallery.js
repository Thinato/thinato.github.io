...
const GALLERY = [
    {
        ...
        description:
            "Hanging off plastic rocks like it is 1999. This is where the example caption would go — write up to four lines about the shot and anything past that gets politely truncated with an ellipsis so the grid stays tidy.",
    },
    {
        src: placeholder("PHOTO 02", "#000000", "#00ffff"),
        title: "Phonk Night",
        description:
            "Ribcage-rattling bass and questionable lighting. Replace this placeholder by dropping a real image into images/gallery/ and pointing src at it.",
    },
    {
    ...
    },
    {
        ...
        description:
            "A photo of a screen of a config I no longer understand. Four lines of glory. Line two. Line three, still going strong. Line four is the last one you will ever see before the ellipsis swoops in and cuts the rest of this rambling description off for good.",
    },
];
...
function buildGrid() {
    if (!GALLERY.length) {
        grid.innerHTML =
            '<div class="gallery-empty">✨ no pictures yet — check back soon!! ✨</div>';
        return;
    }
    GALLERY.forEach((item, idx) => {
        ...
        card.setAttribute(
            "aria-label",
            "Open picture: " + (item.title || "photo " + (idx + 1)),
        );
...
function renderLightbox() {
    ...
    lbCaption.innerHTML =
        (item.title
            ? '<span class="cap-title">' + escapeHtml(item.title) + "</span>"
            : "") + escapeHtml(item.description || "");
